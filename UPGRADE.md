...
新安装站点会通过 `sql/schema.sql` 创建账号验证码和找回密码相关数据表。后台可在以下位置配置邮箱或短信能力：
...
- 邮箱验证码开关和 SMTP 信息
- 手机验证码开关和短信 HTTP 接口
- 验证码有效期、长度、最大错误次数、重发冷却时间

注意：

- SMTP 密码、短信 API Key、Secret、请求头 JSON 不会暴露到前台页面。
- 密钥类输入框留空保存时会保持旧值。
- 手机短信服务商差异较大，当前提供通用 HTTP 接口配置；如果使用阿里云/腾讯云短信，后续可再接专用签名逻辑。
